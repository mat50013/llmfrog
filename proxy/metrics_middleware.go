...
func MetricsMiddleware(pm *ProxyManager) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !found {
			// Check if this might be a downloadable model (contains "/" or ":")
			// If so, we'll try to resolve it after the handler runs
			if strings.Contains(requestedModel, "/") || strings.Contains(requestedModel, ":") {
				// This might be a HuggingFace model that needs downloading
				// Set up metrics writer anyway, we'll resolve the model name later
				writer := &MetricsResponseWriter{
					ResponseWriter: c.Writer,
					metricsRecorder: &MetricsRecorder{
						metricsMonitor: pm.metricsMonitor,
						realModelName:  requestedModel, // Use requested model for now
						startTime:      time.Now(),
					},
				}
				c.Writer = writer
				...
				// After handler completes, try to resolve the real model name again
				// The model might have been downloaded and added to config
				if resolvedName, found := pm.config.RealModelName(requestedModel); found {
					writer.metricsRecorder.realModelName = resolvedName
				}
